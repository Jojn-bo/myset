...
# 打开测试文档
with open('测试文档.txt', encoding='utf-8') as f:  # 加encoding的原因是因为测试文档中，有unicode
    msg = f.read()
...
while x <= len(commodity2):
    ...
    if (len(commodity2)) > 1:
        if len(commodity3[x-1]) == 7:
            ...
            w_message(x, 6, commodity3[x-1][2])
            ...
            w_message(x, 4, commodity3[x-1][3])
